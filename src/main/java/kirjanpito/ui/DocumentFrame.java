...
public class DocumentFrame extends JFrame implements AccountSelectionListener {
	...
	private JMenuItem csvImportMenuItem;
	...
	protected void createMenuBar() {
		...
		menu.addSeparator();
		csvImportMenuItem = SwingUtils.createMenuItem("CSV-tuonti (Procountor)", null, 'C', null, csvImportListener);
		menu.add(csvImportMenuItem);
	...
	/**
	 * CSV/Procountor-tuonti listener
	 */
	private ActionListener csvImportListener = new ActionListener() {
		public void actionPerformed(ActionEvent e) {
			showCSVImport();
		}
	};
	...
	/**
	 * Avaa CSV-tuonti -ikkunan (Procountor-yhteensopiva)
	 */
	public void showCSVImport() {
		if (!saveDocumentIfChanged()) {
			return;
		}
		...
		AppSettings settings = AppSettings.getInstance();
		String url = settings.getString("csv-import.path", "");
		...
		if (dialog.getResult() == JOptionPane.OK_OPTION) {
			try {
				url = dialog.getURL();
				settings.set("csv-import.path", url);

				DataSource dataSource = registry.getDataSource();
				Session sess = dataSource.openSession();
				Period period = registry.getPeriod();

				count = importFromCSV(url, dataSource.getAccountDAO(sess),
					dataSource.getDocumentDAO(sess), dataSource.getEntryDAO(sess), period);
				SwingUtils.showInformationMessage(this,
	...
	/**
	 * Tuo tilitapahtumat Procountor-yhteensopivasta CSV-tiedostosta.
	 *
	 ...
	 * @return         Tuotujen tapahtumien määrä
	 */
	private int importFromCSV(String url, AccountDAO accDAO, DocumentDAO docDAO,
			EntryDAO entryDAO, Period period) throws Exception {
		...
		try {
			...
			while ((nextLine = reader.readNext()) != null) {
				...
				// Tarkistetaan että rivillä on tarpeeksi sarakkeita ja dataa
				if (nextLine.length > 13 && nextLine[7].length() > 0) {
					...
					// Etsitään tili numeron perusteella
					...
					if (accountId == -1) {
						throw new Exception("Tiliä numerolla " + targetAccountNumber + " ei löydy!");
					}

					entry.setDocumentId(docId);
					entry.setAccountId(accountId);
					entry.setDebit(!debet); // Vastakkainen vienti tyyppi
					entry.setAmount(absAmount);
	...
	/**
	 * Apufunktio CSV-tuonnin debuggaukseen. Tulostaa annetun rivin olennaiset tiedot konsoliin.
